import { AccountBrief, Email, EmailGenerationOptions, EmailSequence, EmailType, PriorityContact } from "../types";
import { personaFramework } from "../data/personas";
...
export class EmailGeneratorService {
  ...
  generateEmailSequences(
    ...
    for (const contact of accountBrief.priorityContacts) {
      const emailSequence = this.generateSequenceForContact(
        contact,
        accountBrief,
        options
      );
  ...
  private generateSequenceForContact(
    ...
    const emails: Email[] = [];
    const personaAttributes = personaFramework[contact.persona];
  ...
  private generatePainPointEmail(
    contact: PriorityContact,
    accountBrief: AccountBrief,
    personaAttributes: any,
    options: EmailGenerationOptions
  ): Email {
    // Select relevant pain points for the persona
    const relevantPainPoints = accountBrief.painPoints.filter(painPoint => {
      return personaAttributes.painPoints.some((personaPain: string) => 
        painPoint.toLowerCase().includes(personaPain.toLowerCase().split(' ')[0])
      );
    });
  ...
  private generateObjectionHandlingEmail(
    ...
    return {
      subject,
      body,
      type: EmailType.OBJECTION_HANDLING
    };
  }
